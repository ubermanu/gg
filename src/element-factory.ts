...
export function createNodeHTMLElement(
	tagName: string,
	defaultGameClass: typeof Node,
) {
	return class extends HTMLElement implements GGElement {
		...
		async loadScript(): Promise<typeof Node | undefined> {
			...
			const module: { default?: typeof Node } = await import(
				/* @vite-ignore */ src
			);
		...
		async waitForChildren() {
			const children = Array.from(this.children).filter(
				(child): child is GGElement =>
					child instanceof HTMLElement && "gameObject" in child,
			);
